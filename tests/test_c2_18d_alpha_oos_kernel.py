...
KERNEL_DIR = ROOT / "finetune/kaggle_c2_18d_alpha_oos"
KERNEL = KERNEL_DIR / "c2_18d_alpha_oos.py"
META = KERNEL_DIR / "kernel-metadata.json"


def load_module():
    spec = importlib.util.spec_from_file_location("c2_18d_alpha_oos", KERNEL)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_kernel_is_c2_only_locked_decode_and_does_not_train():
...
def test_dual_t4_metadata_and_budget():
    ...
    assert metadata["id"] == "luckfu/kronos-small-0-1-c2-18d-alpha-oos"
    assert metadata["code_file"] == "c2_18d_alpha_oos.py"
