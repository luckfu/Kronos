...
- `/kronos/sw.js` and `/kronos/manifest.webmanifest` are public static files (no session cookie required). The service worker is allowed for scope `/kronos/` so the UI can be installed to the home screen as a standalone app. Only the HTML/CSS shell is cached; `/kronos/api/*` stays network-first and is not an offline trading data store.
