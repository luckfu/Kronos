"""Finish the last 27 slices of the 1e-5 coverage pass from C3 last.

C3 stopped at 207/234 on the time cap. Exact resume cannot extend that
plan, so this chunk starts a 27-segment stage from C3 last_state, keeps
AdamW, glues LR at 1e-5, and continues seed 20260918 after slice 507.
Does not overwrite the C3 kernel.
"""
import hashlib
import json
import os
...
SOURCE_COMMIT = "efcb2ff9009c1c765f0a63585b53ebab473e2321"
...
def clone_source():
    for attempt in range(1, 4):
        repo = Path(tempfile.mkdtemp(prefix="kronos-c2-wc-1e5-c4-")) / "repo"
        try:
...
def find_parent_output():
    state_file = find_one("**/small_0.1_stage2_c2_best_wc_1e5_c3/checkpoints/last_state.pt")
    ...
    if missing:
        raise RuntimeError(f"C3 output contract is incomplete: {missing}")
    ...
    if completed != EXPECTED_PARENT_SEGMENT or progress.get("status") not in {"stopped", "completed"}:
        raise RuntimeError(f"C3 is not a durable 207-segment parent: {progress}")
    ...
    if int(best.get("segment", -1)) != EXPECTED_PARENT_BEST_SEGMENT:
        raise RuntimeError(f"Unexpected C3 best checkpoint: {best}")
    last_sha = sha256_file(source_root / "checkpoints/last_model/model.safetensors")
    if last_sha != EXPECTED_C3_LAST_SHA:
        raise RuntimeError(f"C3 last sha mismatch: {last_sha}")
...
def main():
    ...
    print(json.dumps({
        ...
        "initialization": "c3_last_state_preserve_adamw_new_27_segment_stage",
    ...
    if int(state.get("next_epoch", -1)) != EXPECTED_PARENT_SEGMENT:
        raise RuntimeError(f"C3 state boundary mismatch: {state.get('next_epoch')}")
    if state.get("scheduler_type") != "warmup_constant":
        raise RuntimeError(f"C3 scheduler mismatch: {state.get('scheduler_type')}")
    if not state.get("optimizer", {}).get("state"):
        raise RuntimeError("C3 checkpoint has no AdamW moment state")
    print(json.dumps({
        "phase": "parent_ready",
