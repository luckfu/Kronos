#!/usr/bin/env bash
# Aggressive 10x-LR continuation from the completed Best@343 checkpoint.
set -euo pipefail

MODE="${1:-diagnostic}"
if [[ "$MODE" != "diagnostic" && "$MODE" != "full" ]]; then
  echo "usage: $0 {diagnostic|full}" >&2
  exit 2
fi
...
RUN_ROOT="$BASE/runs/beta_v1_2_best343_aggressive_1e5_50seg_seed100"
OUTPUT_NAME="a_share_beta_v1_2_best343_aggressive_1e5_120d_to_10d"
...
[[ "$(sha256sum "$PARENT_ROOT/model.safetensors" | awk '{print $1}')" == "$PARENT_SHA" ]] || {
  echo "Best@343 checksum mismatch" >&2
  exit 1
}
...
if [[ "$MODE" == "diagnostic" ]]; then
  [[ ! -e "$OUTPUT_ROOT" ]] || {
    echo "diagnostic refuses existing output: $OUTPUT_ROOT" >&2
    exit 1
  }
  SEGMENTS_THIS_RUN=50
  RESUME=0
else
  [[ -f "$OUTPUT_ROOT/checkpoints/last_state.pt" ]] || {
    echo "full requires diagnostic state" >&2
    exit 1
  }
...
{
  "stage_id": "beta_v1_2_best343_aggressive_1e5_50seg",
  "purpose": "aggressive 10x-LR continuation with a 50-segment diagnostic gate",
  "parent": {"run": "beta_v1_2_best871_full_every_segment_onecycle_1pass_seed100", "checkpoint": "Best@343", "path": "$PARENT_ROOT", "model_sha256": "$PARENT_SHA", "validation_full_loss": 2.4104652404785156},
  "initialization": {"model": "weights_only", "optimizer": "fresh", "scheduler": "one_cycle", "conditioning": "preserved"},
  "training": {"global_segments": 528, "diagnostic_segments": 50, "coverage_passes": 1, "samples_per_segment": 20000, "batch_size": 64, "seed": 100, "gpu": 0, "amp_dtype": "bfloat16", "predictor_learning_rate": 1e-5, "condition_learning_rate": 1e-5, "trainable_predictor": "all"},
  ...
  "diagnostic_gate": {"stop_on_nonfinite": true, "compare_against_parent_loss": 2.4104652404785156, "manual_review_after_segments": 50}
}
EOF
fi
...
echo "[$(date -u +%FT%TZ)] mode=$MODE parent=Best@343 segments_this_run=$SEGMENTS_THIS_RUN peak_lr=1e-5 validation=full-only-24k" | tee -a "$RUN_ROOT/launcher.log"
