#!/usr/bin/env bash
# Continue Beta v1.1 Best@818 with a moderate two-speed LR schedule.
set -euo pipefail
...
RUN_ROOT="$BASE/runs/beta_v1_1_best818_moderate_lr_seed100"
OUTPUT_NAME="a_share_beta_v1_1_best818_moderate_lr_120d_to_10d"
...
{
  "stage_id": "beta_v1_1_best818_moderate_lr",
  ...
  "initialization": {"model": "weights_only", "optimizer": "fresh", "scheduler": "two_speed", "sector_embedding": "preserved", "size_percentile_mlp": "preserved"},
  ...
  "training": {"global_segments": 528, "coverage_passes": 1, "samples_per_segment": 20000, "seed": 100, "gpu": 0, "amp_dtype": "bfloat16", "predictor_peak_lr": 1e-6, "predictor_min_lr": 5e-7, "condition_peak_lr": 3e-6, "condition_min_lr": 1e-6, "scheduler": "two_speed"},
...
export KRONOS_CONDITION_LEARNING_RATE=3e-6
export KRONOS_SCHEDULER_TYPE=two_speed
...
export KRONOS_CONDITION_FAST_DECAY_LR=1.5e-6
...
echo "[$(date -u +%FT%TZ)] mode=$MODE segments_this_run=$SEGMENTS_THIS_RUN parent=Beta-v1.1-Best@818 gpu=0 predictor_peak=1e-6 condition_peak=3e-6 scheduler=two_speed" | tee -a "$RUN_ROOT/launcher.log"
cd "$CODE_ROOT/finetune"
