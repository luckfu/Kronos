"""Decode-side sweep on the sealed 13-date window: sample_count and temperature.

Zero training. Every D10 Rank IC measured so far (C2 best 0.1844, WC 1e-5 C2
best 0.1836, C4 best 0.1792, cosine C1 best 0.1658) came from
`sample_count=1` at `T=0.6` / `top_p=0.9`, so the cross-sectional score is a
single Monte Carlo draw per symbol and the measured IC is attenuated by
sampling noise. This kernel re-decodes two checkpoints that sit at opposite
ends of the val-CE / Rank-IC inversion:

  A  c2_best            val forecast CE 2.2944  D10 Rank IC 0.1844
  B  wc_1e5_c4_best     val forecast CE 2.2729  D10 Rank IC 0.1792

at sample_count 1 / 4 / 16, repeats sample_count=1 under a second seed to
measure the draw-to-draw noise floor, and adds an untruncated
`T=1.0` / `top_p=1.0` arm so the spread across samples can be scored as a
volatility forecast instead of being shrunk by temperature.

Runs on 2x T4: the (arm, date) task list is split round-robin across two
worker processes, one per GPU. Every task reseeds before decoding, so the
split does not change any result.
"""
import gc
import hashlib
import json
...
SOURCE_COMMIT = "c056c21ddd714d42ab169abb30cd26487fa5cbd1"
OUTPUT = Path("/kaggle/working/kronos_decode_sample_sweep")
...
CHECKPOINTS = {
    ...
    "wc_1e5_c4_best": {
        ...
        "baseline_d10_pooled_rank_ic": 0.17918741695116097,
    },
}

# Ordered cheapest-and-most-decisive first so a timeout still leaves the
# attenuation curves complete. Cost is proportional to sample_count.
ARMS = [
    {"name": "c2_best_n1_s1", "checkpoint": "c2_best", "sample_count": 1,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "wc_1e5_c4_best_n1_s1", "checkpoint": "wc_1e5_c4_best", "sample_count": 1,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "c2_best_n1_s2", "checkpoint": "c2_best", "sample_count": 1,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260921},
    {"name": "wc_1e5_c4_best_n1_s2", "checkpoint": "wc_1e5_c4_best", "sample_count": 1,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260921},
    {"name": "c2_best_n4", "checkpoint": "c2_best", "sample_count": 4,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "wc_1e5_c4_best_n4", "checkpoint": "wc_1e5_c4_best", "sample_count": 4,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "c2_best_n16", "checkpoint": "c2_best", "sample_count": 16,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "wc_1e5_c4_best_n16", "checkpoint": "wc_1e5_c4_best", "sample_count": 16,
     "temperature": 0.6, "top_p": 0.9, "seed": 20260906},
    {"name": "c2_best_n8_honest", "checkpoint": "c2_best", "sample_count": 8,
     "temperature": 1.0, "top_p": 1.0, "seed": 20260906},
    {"name": "wc_1e5_c4_best_n8_honest", "checkpoint": "wc_1e5_c4_best", "sample_count": 8,
     "temperature": 1.0, "top_p": 1.0, "seed": 20260906},
]
...
def clone_source():
    for attempt in range(1, 4):
        repo = REPO_DIR if attempt == 1 else Path(tempfile.mkdtemp(prefix="kronos-decode-sweep-")) / "repo"
        try:
            if repo.exists():
...
def parent_main():
    ...
    for key in CHECKPOINTS:
        first, second = pooled(f"{key}_n1_s1"), pooled(f"{key}_n1_s2")
        if first is not None and second is not None:
            noise_floor[key] = {
                "seed_20260906": first,
                "seed_20260921": second,
                "abs_delta": abs(first - second),
            }
        single = [value for value in (first, second) if value is not None]
        points = []
        if single:
            points.append((1, sum(single) / len(single)))
        for count in (4, 16):
            value = pooled(f"{key}_n{count}")
            if value is not None:
                points.append((count, value))
        fitted = attenuation_fit(points)
        if fitted:
            attenuation[key] = fitted
    ...
    write_summary({
        ...
        "question": (
            "Is the 13-date D10 Rank IC ranking an artifact of sample_count=1 decoding, "
            "and does an untruncated distribution give an honest volatility forecast?"
        ),
        "checkpoints": provenance,
        "sample_set": sample_key,
        "signal_start": start,
        ...
        "single_draw_noise_floor": noise_floor,
