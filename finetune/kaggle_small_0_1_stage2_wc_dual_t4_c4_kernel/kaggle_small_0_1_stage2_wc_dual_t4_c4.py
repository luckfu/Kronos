...
MAX_SEGMENTS_PER_RUN = 43
MAX_RUNTIME_SECONDS = 12600  # 3h30m, enough for the remaining 43 segments.
OUTPUT_NAME = "small_0.1_stage2_wc_dual_t4_c4"
EXPECTED_PARENT_SEGMENT = 491
COVERAGE_SEED = "20260910"  # Must match C1 for exact state continuation.
PARENT_KERNEL = "smmt315/kronos-small-0-1-stage2-wc-dual-t4-c3"
TORCH_VERSION = "2.6.0"
...
def find_parent_model() -> Path:
    matches = sorted(Path("/kaggle/input").glob(
        "**/small_0.1_stage2_wc_dual_t4_c3/checkpoints/last_model/model.safetensors"
    ))
    if len(matches) != 1:
        raise RuntimeError(f"Expected one C3 dual-T4 parent last_model, found {len(matches)}: {matches}")
    ...
    if progress_path.is_file():
        ...
        if completed != EXPECTED_PARENT_SEGMENT:
            raise RuntimeError(
                "C3 continuation boundary mismatch: "
