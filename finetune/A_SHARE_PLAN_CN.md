...
2. V5 训练信号使用 2015-01-01 至 2026-07-16；原始行情保留到 2026-07-31 以提供最后 10 个交易日标签。训练过程的验证池从完整训练窗口随机抽样，不把 2026 单独当作训练内验证集；训练完成后再用时间外和股票外数据做独立评估。
...
当前 V5 保持原版 Kronos 的全序列 next-token Loss，不中途修改，作为 120 日上下文基线。下一版 V6 保持数据、120+10 窗口、模型结构和覆盖顺序不变，只改变 Loss 口径：前 120 日历史 token 仍作为因果上下文输入，但默认不参与反向传播；只对未来 10 日对应的 token 计算 `forecast_loss`。

V6 必须分别记录 `history_loss`、`forecast_loss` 和兼容用的 `full_sequence_loss`。最佳 checkpoint 依据验证池的 `forecast_loss` 保存，不能再由被历史位置主导的全序列 Loss 决定。若时间外评估显示明显灾难性遗忘，再单独测试历史辅助权重 `0.02`；不默认使用 `0.2`，因为 `120 × 0.2` 的历史总权重仍高于 `10 × 1.0` 的未来总权重。
