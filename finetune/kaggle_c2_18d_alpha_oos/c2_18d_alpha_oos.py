"""C2-best confirmatory OOS on the 18-day sealed window through 2026-09-03.
...
OUTPUT = Path("/kaggle/working/kronos_c2_18d_alpha_oos")
...
CHECKPOINTS = {
    "c2_best": {
        ...
        "baseline_13d_n1_d10_pooled_rank_ic": 0.18440434213929857,
    },
}
...
def clone_source():
    for attempt in range(1, 4):
        repo = REPO_DIR if attempt == 1 else Path(tempfile.mkdtemp(prefix="kronos-c2-18d-")) / "repo"
        try:
            if repo.exists():
...
def parent_main():
    ...
    write_summary({
        ...
        "question": (
            ...
            "and ranking decode protocols keep D10 Rank IC after industry+size "
            "residualization, and does overlapping ICIR survive Newey-West / "
