"""13-date OOS for this cosine C1 round's best: local seg 148.

Does not re-run C2 init (0.1844), C2-kernel best (0.1836), or C4 best (0.1792).
"""
import gc
import hashlib
import json
...
OUTPUT = Path("/kaggle/working/kronos_small_0_1_stage2_c2_best_wc_1e5_cosine_c1_oos")
...
def clone_source():
    for attempt in range(1, 4):
        repo = Path(tempfile.mkdtemp(prefix="kronos-wc-1e5-cosine-c1-oos-")) / "repo"
        try:
...
def main():
    ...
    if completed != EXPECTED_PARENT_COMPLETED:
        raise RuntimeError(f"Cosine C1 is not a 170-segment parent: {progress}")
    ...
    if int(best_metric.get("segment", -1)) != EXPECTED_LOCAL_SEGMENT:
        raise RuntimeError(f"Unexpected cosine C1 best segment: {best_metric}")
    if not math.isclose(float(best_metric["forecast_loss"]), EXPECTED_FORECAST, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError(f"Unexpected cosine C1 best forecast CE: {best_metric}")
    best_sha = sha256_file(best_file)
    if best_sha != EXPECTED_SHA:
        raise RuntimeError(f"Cosine C1 best sha mismatch: {best_sha}")
    ...
    print(json.dumps({
        ...
        "parent_completed_segments": completed,
        "gpu": gpu,
    ...
    write_summary({
        ...
        "already_measured_not_rerun": [
            "c2_init_0.1844",
            "wc_1e5_c2_best_g289_0.1836",
            "wc_1e5_c4_best_g528_0.1792",
        ],
        "metrics": metrics,
        "remaining": remaining,
    })
