...
if "sm_60" not in torch.cuda.get_arch_list():
    raise RuntimeError(
        f"Installed PyTorch {torch.__version__} lacks P100 sm_60 support: "
        f"{torch.cuda.get_arch_list()}"
    )
print(
    {"torch": torch.__version__, "cuda_arches": torch.cuda.get_arch_list()},
    flush=True,
)
...
os.environ.update({
    ...
    "SWANLAB_EXPERIMENT_NAME": "small_0.1_bootstrap_timing",
})
subprocess.run(
