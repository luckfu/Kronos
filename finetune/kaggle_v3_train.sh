...
if [[ -f "${PATHS_FILE}" ]]; then
  # Generated by kaggle_v3_bootstrap.sh to preserve detected read-only paths.
  source "${PATHS_FILE}"
fi
...
OUTPUT_NAME="${KRONOS_PREDICTOR_SAVE_FOLDER:-a_share_size_full_market_v3_kaggle_benchmark}"
OUTPUT_ROOT="${RUNTIME_ROOT}/outputs/models/${OUTPUT_NAME}"
RESUME_PATH="${OUTPUT_ROOT}/checkpoints/last_state.pt"
CONFIG_LOCK="${OUTPUT_ROOT}/kaggle_v3_run_config.env"
...
export KRONOS_EARLY_STOPPING_PATIENCE="${KRONOS_EARLY_STOPPING_PATIENCE:-0}"
...
trap 'rm -f "${CURRENT_CONFIG}"' EXIT
{
  ...
  echo "epochs=${KRONOS_EPOCHS}"
  echo "require_full_coverage=${KRONOS_REQUIRE_FULL_COVERAGE}"
...
if [[ -f "${CONFIG_LOCK}" ]]; then
  if ! cmp -s "${CURRENT_CONFIG}" "${CONFIG_LOCK}"; then
    echo "[kaggle-v3] ERROR: resume configuration differs from ${CONFIG_LOCK}" >&2
...
if [[ "${KRONOS_RESUME_TRAINING}" == "1" && ! -f "${RESUME_PATH}" ]]; then
  echo "[kaggle-v3] ERROR: resume requested but ${RESUME_PATH} does not exist" >&2
  exit 1
fi
if [[ "${KRONOS_RESUME_TRAINING}" == "0" && -f "${RESUME_PATH}" ]]; then
  echo "[kaggle-v3] ERROR: output already contains a checkpoint; resume or choose a new output" >&2
  exit 1
fi
