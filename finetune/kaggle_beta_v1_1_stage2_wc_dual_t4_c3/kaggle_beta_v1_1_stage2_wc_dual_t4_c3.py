"""Continue Beta v1.1 Stage2 segments 91-170 from the verified C2 output."""
...
def validate_parent(input_root):
    ...
    if len(matches) != 1:
        raise RuntimeError(f"Expected exactly one C2 checkpoint, found {len(matches)}")
    ...
    if missing:
        raise RuntimeError(f"C2 output incomplete: {missing}")
    ...
    expected = {
        ...
        "progress_windows": (progress.get("unique_windows_covered"), 1800000),
    ...
    if wrong:
        raise RuntimeError(f"C2 lineage mismatch: {wrong}")
    print(
        json.dumps(
            {
                "phase": "parent_verified",
...
def load_runner():
    ...
    namespace = {"__name__": "kronos_beta_v1_1_stage2_runner", "__file__": SOURCE_PATH}
...
def pinned_clone(original_clone):
    def clone(repo):
        original_clone(repo)
        subprocess.run(
            ["git", "-C", str(repo), "fetch", "--depth", "1", "origin", TRAIN_COMMIT],
            check=True,
        )
        subprocess.run(
...
class TimingRun:
    ...
    def log(self, payload, **kwargs):
        if "timing/segment_seconds" in payload:
            ...
            payload = {
                **payload,
                "timing/avg_segment_seconds": self.total_seconds / self.count,
            }
