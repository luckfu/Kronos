...
# Validation is a random sample from the same complete training pool. The
# trainer applies these boundaries when it enumerates validation windows.
...
COVERAGE_PASSES="${KRONOS_COVERAGE_PASSES:-2}"
export KRONOS_COVERAGE_PASSES="${COVERAGE_PASSES}"
export KRONOS_EPOCHS="${KRONOS_EPOCHS:-${COVERAGE_PASSES}}"
