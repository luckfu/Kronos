...
### V4 A/B 实测结论（2026-08-06）

P100 任务已完成两组训练和 4,784 条时间外预测。验证集 token loss 为 A `2.7442`、B `2.7459`，但生产晋级以未见日期的横截面指标为准：

| 模型 | Rank IC | Rank IC 为正的信号日 | 方向准确率 | MAE | 预测下跌比例 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 增训前 V3 Last | `0.01124` | 50.0% | 58.95% | 0.06129 | 66.43% |
| V4 A：修正近期 | `-0.04796` | 30.0% | 64.23% | 0.06200 | 81.63% |
| V4 B：修正近期 + 20% 回放 | `-0.03966` | 35.0% | 62.90% | 0.06206 | 78.87% |

这 20 个信号日实际下跌比例为 `73.37%`。A/B 的方向准确率看似更高，主要来自更多地预测下跌；两者的平衡准确率都低于 `0.50`，Rank IC 反而显著低于 V3 Last。B 比 A 稍微减轻了向下偏移，但不足以挽回排序能力。因此本次只确认了数据裁剪问题并保留了可复现的修正管线，两个候选不替换当前生产模型，也不上传 ModelScope。

当前生产模型在这 20 个日期上的结果不能作为公平基准，因为它的训练曾经见过这些日期的非 holdout 股票；该结果仅用于说明股票隔离与时间隔离的区别，不用于晋级判断。
