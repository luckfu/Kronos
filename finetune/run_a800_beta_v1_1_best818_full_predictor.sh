#!/usr/bin/env bash
# Full-Predictor incremental fine-tuning control run from Beta v1.1 Best@818.
set -euo pipefail
...
PARENT_ROOT="$SOURCE_OUTPUT/checkpoints/best_model"
TOKENIZER_ROOT="$BASE/models/tokenizer"
RUN_ROOT="$BASE/runs/beta_v1_1_best818_full_predictor_bs64_seed100"
OUTPUT_NAME="a_share_beta_v1_1_best818_full_predictor_bs64_120d_to_10d"
...
PARENT_SHA="b890771368737c6c93825165695afc16b57870f4692f87a563392cc96e405673"
...
{
  "stage_id": "beta_v1_1_best818_full_predictor_bs64",
  "purpose": "full Predictor incremental fine-tuning control from Beta v1.1 Best@818",
  "execution": "single_gpu_a800_offline",
  "parent": {"release": "Beta v1.1", "checkpoint": "Best@818", "path": "$PARENT_ROOT", "model_sha256": "$PARENT_SHA"},
  "initialization": {"model": "weights_only", "optimizer": "fresh", "scheduler": "one_cycle", "sector_embedding": "preserved", "size_percentile_mlp": "preserved"},
  ...
  "training": {"basemodel_epochs": 1, "global_segments": 1056, "coverage_passes": 2, "samples_per_segment": 20000, "batch_size": 64, "seed": 100, "gpu": 0, "amp_dtype": "bfloat16", "predictor_learning_rate": 1e-6, "scheduler": "one_cycle", "trainable_predictor": "all"},
  "selection_policy": "Do not use the sealed 2026-08 future set for checkpoint selection or hyperparameter changes",
  "known_limitation": "This stage loads Beta v1.1 Best@818 model weights only, preserves both conditioning branches, and starts a fresh unified OneCycle optimizer/scheduler with a 1e-6 peak learning rate"
}
EOF
fi
...
export KRONOS_SCHEDULER_TYPE=one_cycle
export KRONOS_SCHEDULER=one_cycle
export KRONOS_PREDICTOR_WARMUP_START_LR=1e-7
export KRONOS_CONDITION_WARMUP_START_LR=1e-7
export KRONOS_PREDICTOR_MIN_LR=1e-8
export KRONOS_CONDITION_MIN_LR=1e-8
...
echo "[$(date -u +%FT%TZ)] mode=$MODE segments_this_run=$SEGMENTS_THIS_RUN parent=Beta-v1.1-Best@818 gpu=0 trainable_predictor=all batch_size=64 predictor_lr=1e-6 scheduler=one_cycle" | tee -a "$RUN_ROOT/launcher.log"
cd "$CODE_ROOT/finetune"
