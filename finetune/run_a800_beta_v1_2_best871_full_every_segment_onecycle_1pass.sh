#!/usr/bin/env bash
# One-coverage full-Predictor continuation from Beta v1.2 Best@871.
set -euo pipefail

MODE="${1:-dry-run}"
if [[ "$MODE" != "dry-run" && "$MODE" != "diagnostic" && "$MODE" != "full" ]]; then
  echo "usage: $0 {dry-run|diagnostic|full}" >&2
  exit 2
fi
...
PARENT_RUN="$BASE/runs/beta_v1_1_best818_full_predictor_bs64_seed100"
PARENT_OUTPUT="$PARENT_RUN/outputs/models/a_share_beta_v1_1_best818_full_predictor_bs64_120d_to_10d"
PARENT_ROOT="$PARENT_OUTPUT/checkpoints/best_model"
TOKENIZER_ROOT="$BASE/models/tokenizer"
RUN_ROOT="$BASE/runs/beta_v1_2_best871_full_every_segment_onecycle_1pass_seed100"
OUTPUT_NAME="a_share_beta_v1_2_best871_full_every_segment_onecycle_1pass_120d_to_10d"
...
PARENT_SHA="965c3bc40f6ca17d2fbff16bcb7b1c15ab4ac5d47204d5714108b813462393dd"
...
[[ "$(sha256sum "$PARENT_ROOT/model.safetensors" | awk '{print $1}')" == "$PARENT_SHA" ]] || {
  echo "Beta v1.2 Best@871 checksum mismatch" >&2
  exit 1
}
...
if [[ "$MODE" == "dry-run" ]]; then
  [[ ! -e "$OUTPUT_ROOT" ]] || { echo "dry-run refuses existing output: $OUTPUT_ROOT" >&2; exit 1; }
  SEGMENTS_THIS_RUN=1
  RESUME=0
elif [[ "$MODE" == "diagnostic" ]]; then
  [[ -f "$OUTPUT_ROOT/checkpoints/last_state.pt" ]] || { echo "diagnostic requires dry-run state" >&2; exit 1; }
  SEGMENTS_THIS_RUN=49
  RESUME=1
else
  [[ -f "$OUTPUT_ROOT/checkpoints/last_state.pt" ]] || { echo "full requires resumable state" >&2; exit 1; }
  SEGMENTS_THIS_RUN=478
  RESUME=1
fi
...
{
  "stage_id": "beta_v1_2_best871_full_every_segment_onecycle_1pass",
  "purpose": "one-coverage continuation from Beta v1.2 Best@871 with fixed 24k validation after every segment",
  "parent": {"release": "Beta v1.2", "checkpoint": "Best@871", "path": "$PARENT_ROOT", "model_sha256": "$PARENT_SHA"},
  "initialization": {"model": "weights_only", "optimizer": "fresh", "scheduler": "one_cycle", "conditioning": "preserved"},
  "training": {"global_segments": 528, "coverage_passes": 1, "samples_per_segment": 20000, "batch_size": 64, "seed": 100, "gpu": 0, "amp_dtype": "bfloat16", "predictor_learning_rate": 1e-6, "condition_learning_rate": 1e-6, "trainable_predictor": "all"},
  ...
  "selection_policy": "The sealed 2026-08 future set is not used for checkpoint selection or hyperparameter changes"
}
EOF
fi
...
# The sealed manifest records a historical 6k prefix. Full-only mode keeps
# this value solely for manifest integrity checks and never builds that loader.
...
export KRONOS_PREDICTOR_LEARNING_RATE=1e-6 KRONOS_CONDITION_LEARNING_RATE=1e-6
export KRONOS_SCHEDULER_TYPE=one_cycle KRONOS_SCHEDULER=one_cycle
export KRONOS_PREDICTOR_WARMUP_START_LR=1e-7 KRONOS_CONDITION_WARMUP_START_LR=1e-7
...
export KRONOS_BOOTSTRAP_COMPLETED_SEGMENTS=0 KRONOS_MAX_SEGMENTS_PER_RUN="$SEGMENTS_THIS_RUN"
export KRONOS_RESUME_TRAINING="$RESUME" PYTHONUNBUFFERED=1

echo "[$(date -u +%FT%TZ)] mode=$MODE segments_this_run=$SEGMENTS_THIS_RUN parent=Beta-v1.2-Best@871 coverage=1 peak_lr=1e-6 validation=full-only-24k-every-segment best=validation/full/loss" | tee -a "$RUN_ROOT/launcher.log"
