"""Continue Stage2 warmup-constant 1e-5 for ~10 more hours.

C1 finished 100/100, so exact resume cannot extend the plan. This chunk
starts a new 200-segment stage from C1 last_state, keeps AdamW, glues LR
at 1e-5, and continues the same coverage permutation after slice 100.
C1 measured 100 segments in 5 hours, so 200 segments is the 10-hour budget.
"""
import hashlib
import json
import os
...
SOURCE_COMMIT = "47ba6280cccd0a7633e6055eded602867974298a"
OUTPUT_NAME = "small_0.1_stage2_c2_best_wc_1e5_c2"
SWANLAB_RUN_ID = "small_0.1_stage2_c2_best_wc_1e5_c2"
...
def clone_source():
    for attempt in range(1, 4):
        repo = Path(tempfile.mkdtemp(prefix="kronos-c2-wc-1e5-c2-")) / "repo"
        try:
...
def find_parent_output():
    state_file = find_one("**/small_0.1_stage2_c2_best_wc_1e5/checkpoints/last_state.pt")
    ...
    if missing:
        raise RuntimeError(f"C1 output contract is incomplete: {missing}")
    ...
    if completed != EXPECTED_PARENT_SEGMENT or progress.get("status") != "completed":
        raise RuntimeError(f"C1 is not a completed 100-segment parent: {progress}")
    ...
    if int(best.get("segment", -1)) != EXPECTED_PARENT_BEST_SEGMENT:
        raise RuntimeError(f"Unexpected C1 best checkpoint: {best}")
    last_sha = sha256_file(source_root / "checkpoints/last_model/model.safetensors")
    if last_sha != EXPECTED_C1_LAST_SHA:
        raise RuntimeError(f"C1 last sha mismatch: {last_sha}")
...
def main():
    ...
    print(json.dumps({
        ...
        "initialization": "c1_last_state_preserve_adamw_new_200_segment_stage",
    ...
    if state.get("scheduler_type") != "warmup_constant":
        raise RuntimeError(f"C1 scheduler mismatch: {state.get('scheduler_type')}")
    if not state.get("optimizer", {}).get("state"):
        raise RuntimeError("C1 checkpoint has no AdamW moment state")
    print(json.dumps({
        "phase": "parent_ready",
