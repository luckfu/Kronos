#!/usr/bin/env bash
# Continue Beta v1.1 Last@1058 for two coverage passes at half the prior peak LRs.
set -euo pipefail
...
RUN_ROOT="$BASE/runs/beta_v1_1_last1058_half_lr_2pass_seed100"
OUTPUT_NAME="a_share_beta_v1_1_last1058_half_lr_2pass_120d_to_10d"
...
{
  "stage_id": "beta_v1_1_last1058_half_lr_2pass",
  "purpose": "test whether terminal-loss training continues to improve or overfits",
  "execution": "single_gpu_a800_offline",
  "parent": {"release": "Beta v1.1", "checkpoint": "Last@1058", "path": "$PARENT_ROOT", "model_sha256": "$PARENT_SHA"},
  "initialization": {"model": "weights_only", "optimizer": "fresh", "scheduler": "fresh_two_speed_half_prior_peak", "sector_embedding": "preserved", "size_percentile_mlp": "preserved"},
  ...
  "training": {"global_segments": 1058, "coverage_passes": 2, "samples_per_segment": 20000, "seed": 100, "gpu": 0, "amp_dtype": "bfloat16", "predictor_peak_lr": 1.5e-6, "predictor_min_lr": 2.5e-7, "condition_peak_lr": 1.5e-5, "condition_fast_decay_lr": 5e-6, "condition_min_lr": 5e-7, "scheduler": "two_speed"},
...
export KRONOS_PREDICTOR_LEARNING_RATE=1.5e-6
export KRONOS_CONDITION_LEARNING_RATE=1.5e-5
export KRONOS_SCHEDULER=two_speed
export KRONOS_SCHEDULER_MIN_LR=5e-7
export KRONOS_PREDICTOR_MIN_LR=2.5e-7
export KRONOS_CONDITION_MIN_LR=5e-7
export KRONOS_SCHEDULER_WARMUP_RATIO=0.005
export KRONOS_PREDICTOR_WARMUP_START_LR=5e-7
export KRONOS_CONDITION_WARMUP_START_LR=5e-6
export KRONOS_CONDITION_FAST_DECAY_RATIO=0.075
export KRONOS_CONDITION_FAST_DECAY_LR=5e-6
...
echo "[$(date -u +%FT%TZ)] mode=$MODE segments_this_run=$SEGMENTS_THIS_RUN parent=Beta-v1.1-Last@1058 gpu=0" | tee -a "$RUN_ROOT/launcher.log"
cd "$CODE_ROOT/finetune"
